...
QVariant TicketModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == Qt::DisplayRole) {
        ...
        case 6: return t.createdAt.toString("dd:MM:yyyy");
        case 7: return t.updatedAt.toString("dd:MM:yyyy");
        }
    }
    if (role == Qt::UserRole) {
...
void TicketBadgeDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    if (index.column() == 2) { // Status only
        QString value = index.data(Qt::DisplayRole).toString();
        QRect r = option.rect;
        QColor border = Qt::gray;
        QColor fg = Qt::black;
        if (value == "Opened" || value == "Open") { border = QColor("#27ae60"); fg = Qt::black; value = "Open"; }
        else if (value == "Closed") { border = QColor("#e74c3c"); fg = Qt::black; }
        else if (value == "In Progress") { border = QColor("#2980b9"); fg = Qt::black; }
        painter->save();
